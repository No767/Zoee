...
class ErrorHandler(commands.Cog):
    ...
    def build_error_embed(self, error: AppCommandError) -> ErrorEmbed:
        ...
        embed = ErrorEmbed()
        embed.description = f"""
        Uh oh! It seems like the command ran into an issue! For support, please ask the devs
    ...
    def build_missing_perm_embed(
        ...
        if is_bot:
            desc = f"""
            It looks like the bot is missing these permissions:
            {','.join(missing_perms).rstrip(',')}
            """
    ...
    # Since this is a slash command only bot,
    # we want to supress all errors coming from prefixed commands
